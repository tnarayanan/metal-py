...
RetCode mc_err(RetCode ret) {
    ...
    if (ret != Success) {
        ...
        switch (ret) {
            ...
            // Python level errors
            case FirstArgumentNotDevice: errString = "First argument should be a metal.Device object"; break;
            case FirstArgumentNotKernel: errString = "First argument should be a metal.Kernel object"; break;
        ...
        if (ret == FailedToCompile) {
            ...
            PyErr_SetString(MetalError, compileErrString);
            free(compileErrString);
        } else {
            PyErr_SetString(MetalError, errString);
        }
...
{
    return PyUnicode_FromFormat("metal.Device(%s)", self->dev_handle.name);
}
...
{
    return PyUnicode_FromFormat("metal.Kernel");
}
...
{
    return PyUnicode_FromFormat("metal.Function");
}
...
{
    return PyUnicode_FromFormat("metal.Buffer(length=%lld)",self->length);
}
...
int to_buffer(PyObject* possible_buffer, Device* dev, Buffer** buffer) {
    // The input is either
    // 1. Already a metal Buffer*, if so give and return 0;
    // 2. A python object which can expose a buffer
    //    If so create and give a temporary metal buffer with a copy of the data and return 0
...
Run_str(Run* self)
{
    return PyUnicode_FromFormat("metal.Run");
}
...
PyInit_metal(void)
{
    ...
    m = PyModule_Create(&metalmodule);
    if (m == NULL)
        return NULL;

    MetalError = PyErr_NewException("metal.error", NULL, NULL);
    Py_XINCREF(MetalError);
    if (PyModule_AddObject(m, "error", MetalError) < 0) {
        Py_XDECREF(MetalError);
        Py_CLEAR(MetalError);
        Py_DECREF(m);
        return NULL;
    }
